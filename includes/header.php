...
<meta name="description" content="Mischief Outlaws Motorcycle Club. A disciplined riding club with a place for the road, the garage, and the people who keep both moving.">
<title><?= e($title ?? 'Mischief Outlaws MC') ?></title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;500;600;700&family=DM+Sans:wght@400;500;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="assets/css/app.css">
</head>
<body>
<a class="skip-link" href="#main">Skip to content</a>
<div class="custom-cursor-dot" aria-hidden="true"></div><div class="custom-cursor-ring" aria-hidden="true"></div>
<header class="site-header"><div class="shell nav-wrap"><a class="brand" href="<?= url() ?>"><span class="brand-mark">MO</span><span>Mischief<br><b>Outlaws</b></span></a><button class="nav-toggle" aria-expanded="false" aria-controls="nav">Menu</button><nav id="nav" class="nav-links" aria-label="Primary navigation"><?php foreach ($navItems as $item): $isActive = $activeRoute === $item['route']; ?><a class="nav-link <?= $isActive ? 'is-active' : '' ?> <?= $item['route'] === 'prospect' ? 'nav-cta' : '' ?>" href="<?= url($item['route']) ?>" <?= $isActive ? 'aria-current="page"' : '' ?>><?= e($item['label']) ?></a><?php endforeach; ?></nav></div></header>
